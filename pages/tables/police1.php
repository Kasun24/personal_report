...
if (isset($_GET['delete'])) {
    $apSMLNo = $_GET['delete'];
    $sql = "select * from area_police where apSMLNo = " . $apSMLNo;
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $apDoc = $row['apDoc'];
        unlink($upload_dir . $apDoc);

        $sql = "delete from area_police where apSMLNo=" . $apSMLNo;
        if (mysqli_query($conn, $sql)) {
            header('location:police1.php');
        }
    }
}
?>
        ...
        <div class="container-fluid page-body-wrapper">
            ...
            <div class="main-panel">
                ...
                <div class="content-wrapper">
                    <h4 align="center">Area Police Report Details Table</h4>
                    <br>
                    <table id="example" class="table table-fixed table-bordered table-responsive table-hover" style="width:100%">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th width="10%" style="padding:10px">Area Police<br>Name</th>
                                <th width="10%" style="padding:10px">Document<br>Issued Date</th>
                                <th width="55%" style="padding:10px">If Any Police Problem<br>(Describe It)</th>
                                <th width="10%" style="padding:10px">Recived<br>Date</th>
                        ...
                        <tbody>
                            <?php
                            $sql = "select * from area_police";
                            $result = mysqli_query($conn, $sql);
                            if (mysqli_num_rows($result)) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ?>
                                    <tr>
                                        <td width="5%" style="padding:10px"><?php echo $row['apSMLNo'] ?></td>
                                        <td width="10%" style="padding:10px"><?php echo $row['apName'] ?></td>
                                        <td width="10%" style="padding:10px"><?php echo $row['apDID'] ?></td>
                                        <td width="55%" style="padding:10px"><?php echo $row['apProb'] ?></td>
                                        <td width="10%" style="padding:10px"><?php echo $row['apRecD'] ?></td>
                                        <td width="5%"><a href="<?php echo $upload_dir . $row['apDoc'] ?>"><img src="<?php echo $upload_dir . $row['apDoc'] ?>" style="border-radius: 0%;"></a></td>
                                        <td width="5%" class="text-center" style="padding:10px">
                                            <a href="police1.php?delete=<?php echo $row['apSMLNo'] ?>" class="btn btn-danger" style="padding: 5px 10px;" onclick="return confirm('Are you sure to delete this record?')">Delete</a><br>
                                            <a href="../edits/editpolice1.php?apSMLNo=<?php echo $row['apSMLNo'] ?>" style="padding: 5px 10px;"   class="btn btn-warning">Edit</a>
                                        </td>
                                    </tr>
                            <?php
                                }
