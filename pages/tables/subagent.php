...
if (isset($_GET['delete'])) {
    $sub_sml = $_GET['delete'];
    $sql = "select * from sub_agent where sub_sml = " . $sub_sml;
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $sql = "delete from sub_agent where sub_sml=" . $sub_sml;
        if (mysqli_query($conn, $sql)) {
            header('location:subagent.php');
        }
    }
}
?>
        ...
        <div class="container-fluid page-body-wrapper">
            ...
            <div class="main-panel">
                ...
                <div class="content-wrapper">
                    <h4 align="center">Sub Agent Details Table</h4>
                    <br>
                    <table id="example" class="table table-fixed table-bordered table-responsive table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th width="15%" style="padding:10px">SML No</th>
                                <th width="20%" style="padding:10px">Sub Agent<br>Code</th>
                                <th width="20%" style="padding:10px">Sub Agent<br>Name</th>
                                <th width="35%" style="padding:10px">Sub Agent<br>Address</th>
                                <th width="10%" style="padding:10px">Sub Agent<br>Contact No 1</th>
                                <th width="35%" style="padding:10px">Sub Agent<br>Contact No 2</th>
                                <th width="10%" style="padding:10px">Sub Agent<br>Id No</th>
                                <th width="10%" style="padding:10px">Action</th>
                            </tr>
                        </thead>

                        <tbody>
                            <?php
                            $sql = "select * from sub_agent";
                            $result = mysqli_query($conn, $sql);
                            if (mysqli_num_rows($result)) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ?>
                                    <tr>
                                        <td width="15%" style="padding:10px"><?php echo $row['sub_sml'] ?></td>
                                        <td width="20%" style="padding:10px"><?php echo $row['sub_code'] ?></td>
                                        <td width="20%" style="padding:10px"><?php echo $row['sub_name'] ?></td>
                                        <td width="35%" style="padding:10px"><?php echo $row['sub_addr'] ?></td> 
                                        <td width="10%" style="padding:10px"><?php echo $row['sub_cont1'] ?></td> 
                                        <td width="20%" style="padding:10px"><?php echo $row['sub_cont2'] ?></td>
                                        <td width="35%" style="padding:10px"><?php echo $row['sub_id'] ?></td> 
                                        <td width="10%" style="padding:10px">                                  
                                            <a href="subagent.php?delete=<?php echo $row['sub_sml'] ?>" class="btn btn-danger" style="padding: 5px 10px;" onclick="return confirm('Are you sure to delete this record?')">Delete</a><br>
                                            <a href="../edits/editsubagent.php?sub_sml=<?php echo $row['sub_sml'] ?>" style="padding: 5px 10px;"   class="btn btn-warning">Edit</a>
                                        </td>
                                    </tr>
                            <?php
                                }
