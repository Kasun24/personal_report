...
if (isset($_GET['delete'])) {
    $hqSMLNo = $_GET['delete'];
    $sql = "select * from head_police where hqSMLNo = " . $hqSMLNo;
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $hqDoc = $row['hqDoc'];
        unlink($upload_dir . $hqDoc);

        $sql = "delete from head_police where hqSMLNo=" . $hqSMLNo;
        ...
        <div class="container-fluid page-body-wrapper">
            ...
            <div class="main-panel">
                ...
                <div class="content-wrapper">
                    <h4 align="center">Head Quarters Police Report Details Table</h4>
                    <br>
                    <table id="example" class="table table-fixed table-bordered table-responsive table-hover" style="width:100%">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th width="10%" style="padding:10px">Apply Date</th>
                                <th width="10%" style="padding:10px">Reference<br>No</th>
                        ...
                        <tbody>
                            <?php
                            $sql = "select * from head_police";
                            $result = mysqli_query($conn, $sql);
                            if (mysqli_num_rows($result)) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ?>
                                    <tr>
                                        <td width="5%" style="padding:10px"><?php echo $row['hqSMLNo'] ?></td>
                                        <td width="10%" style="padding:10px"><?php echo $row['hqApplyD'] ?></td>
                                        <td width="10%" style="padding:10px"><?php echo $row['hqRefNo'] ?></td>
                                        <td width="55%" style="padding:10px"><?php echo $row['hqProb'] ?></td>
                                        <td width="10%" style="padding:10px"><?php echo $row['hqRecD'] ?></td>
                                        <td width="5%"><a href="<?php echo $upload_dir . $row['hqDoc'] ?>"><img src="<?php echo $upload_dir . $row['hqDoc'] ?>" style="border-radius: 0%;"></a></td>
                                        <td width="5%" class="text-center" style="padding:10px">
                                            <a href="police2.php?delete=<?php echo $row['hqSMLNo'] ?>" class="btn btn-danger" style="padding: 5px 10px;" onclick="return confirm('Are you sure to delete this record?')">Delete</a><br>
                                            <a href="../edits/editpolice2.php?hqSMLNo=<?php echo $row['hqSMLNo'] ?>" style="padding: 5px 10px;" class="btn btn-warning">Edit</a>
                                        </td>
                                    </tr>
                            <?php
                                }
